...
public:
	...
	bool findPlayer(string name)
	{
		...
		while (currentNode != NULL)  ////As long as currentNode points to something, it will do the while loop.
		{
			if (currentNode->data->getName() == name)  //// If the currentNodes data == value passed in, then it returns true.
			{
				return true;
			}
			else   //// If the current node does not == value, then it makes currentNode point to the next node.
	...
	int findIndex(string name)
	{
		...
		//return index; //// If currentNode-> data never == value, then it returns false. 
	}
		...
		*/
			...
			else  ////If count doesnt == 0, it does this
			{
				if (count > 0) //// If count > 0, it does this. If count < 0, it does nothing.
				{
					...
					nh->prev = NULL; ////Since this is the new head, there is none before it.
					head = nh;  //// Sets the head pointer to point to this new node.
					count++;    //// Increases count of nodes in Linked list by 1.
				}
			}
				...
				if (count > 0) //// If count > 0, it does this. If count < 0, it does nothing.
				{
					while (currentNode != NULL)////As long as currentNode points to something, it will do the while loop.
					{
						if (currentNode->next == NULL)////If currentNode->next == NULL, it means it is the tail node.
						{
							...
							nt->prev = tail; //Makes the nt->prev point to the tail.
	...
	string toString()
	{
		...
		while (currentNode != NULL)
		{
			if (index > 0)
			{
				...
				{
					RosterList += currentNode->data->getName();
					RosterList += " ";
					currentNode = currentNode->next;
					index--;
				}
	...
	void ShuffleList()
	{
		...
		{
			int swapped = rand() % 1000 / 1000.0;

			if (swapped < swapPossibility)
			{
